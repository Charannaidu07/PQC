...
def compute_optimal_kem(cpu, ram, battery, threat_score, stats):
    ...
    """
    # -------------------------------------------------------------------------
    # HARD RESOURCE CONSTRAINT OVERRIDE:
    # Devices with RAM < 512 MB are hard-constrained to ML-KEM-512 because the
    # heavier ML-KEM-768 requires larger stack/heap footprint that exceeds the 
    # memory safety thresholds of constrained microcontrollers, bypassing policy.
    # -------------------------------------------------------------------------
    if ram < 512:
    ...
    # -------------------------------------------------------------------------
    # UTILITY WEIGHTS CALIBRATION METHODOLOGY (KEM):
    # These coefficients (10.0, 0.05, 0.005, 2.0) are policy parameters calibrated 
    # via grid-search optimization to normalize decapsulation latency, security,
    # and bandwidth parameters, ensuring both algorithms are viable candidates.
    # -------------------------------------------------------------------------
...
def compute_optimal_sig(cpu, ram, battery, threat_score, stats):
    ...
    # -------------------------------------------------------------------------
    # UTILITY WEIGHTS CALIBRATION METHODOLOGY (Signatures):
    # Manually tuning these weights could introduce selection bias. To prevent 
    # this, the coefficients (16.5699, 0.1503, 0.0239, 0.0054, 0.8450) were tuned 
    # using a constraint optimization grid search. The optimization goal was to 
    # achieve a mathematically balanced (approx. 50/50 class split) label 
    # distribution over the simulated scenario space. This calibrates the relative 
    # cost offsets between ML-DSA-44 and FN-DSA-512, establishing a balanced policy.
    # -------------------------------------------------------------------------
...
# ==========================================
# TRAIN (RandomForest acts as a policy approximation surrogate)
# ==========================================

def train_selector():
    """
    Trains the Random Forest classifiers.
    METHODOLOGICAL NOTE: The Machine Learning model learns to approximate (distill) 
    the optimal policy decisions generated by our multi-criteria utility optimization 
    formulas. This acts as an efficient runtime surrogate for policy execution, 
    bypassing the need to run the full floating-point optimization mathematical solver 
    on constrained IoT edge hardware.
    """
